...
void decrypt(int ciphertext[8], int key[10])
{
    ...
    initialPermutation(ciphertext, initialPermResult);

    // Round 1
    printf("Round 1:\n");
    binaryXOR(initialPermResult, subkey_K2, xorResult);
    ...
    for (int i = 0; i < 4; i++)
    {
        xorResult[i] = initialPermResult[i];
        xorResult[i + 4] = replacedResult[i];
    }
...
int main()
{
    int ciphertext[8] = {1, 1, 0, 0, 0, 1, 0, 1};
